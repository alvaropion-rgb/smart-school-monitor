...
function uploadImageToDrive(blueprintId, imageDataUrl) {
  return new Promise(function(resolve, reject) {
    window.api.invoke('saveBlueprintImage', blueprintId, imageDataUrl).then(function(result) {
...
function fetchImageFromDrive(driveFileId, blueprintId) {
  return new Promise(function(resolve) {
    window.api.invoke('getBlueprintImage', driveFileId).then(function(result) {
  ...
  for (var i = 0; i < localBlueprints.length; i++) {
    ...
    try {
      ...
      await new Promise(function(resolve) {
        window.api.invoke('saveBlueprint', {
            id: bpId,
            name: bp.name || bpId,
...
async function initializeApp() {
  try {
    ...
    // window.api is available via Electron preload.js
...
async function loadDevices() {
  return new Promise(function(resolve, reject) {
    window.api.invoke('getDevices').then(function(devices) {
        try {
...
async function loadTraps() {
  return new Promise(function(resolve, reject) {
    window.api.invoke('getTraps', 100).then(function(traps) {
        try {
...
async function loadEmailConfig() {
  return new Promise((resolve, reject) => {
    window.api.invoke('getEmailConfig').then(config => {
...
async function loadBlueprints() {
  return new Promise((resolve, reject) => {
    window.api.invoke('getBlueprints').then(function(blueprints) {
        try {
          var bps = blueprints || [];
...
async function loadTechnicians() {
  return new Promise(function(resolve, reject) {
    window.api.invoke('getTechnicians').then(function(techs) {
...
async function checkPasswordProtection() {
  return new Promise(function(resolve) {
    window.api.invoke('isPasswordProtected').then(function(result) {
...
function goHome() {
  // Navigate back to the main app view (Blueprint tab)
  switchTab('blueprint');
...
function saveTheme() {
  ...
  window.api.invoke('saveSetting', 'customTheme', JSON.stringify(customTheme)).then(function(result) {
...
function saveIconSettings() {
  try {
    ...
    // Save to server
    window.api.invoke('saveSettings', { icons: toSave })
      .then(function() {
...
function verifyPasswordForTab(targetTab) {
  ...
  window.api.invoke('verifySecurityPassword', password).then(function(result) {
      if (result.valid) {
        state.isAuthenticated = true;
...
function switchSettingsPanel(sectionName) {
  ...
  // Lazy-load import status when panel is opened
  if (sectionName === 'data-import') {
    refreshImportStatus();
  }
  ...
  // Lazy-load request page branding when panel is opened
  if (sectionName === 'request-page-branding') {
    loadRequestPageBranding();
  }
...
function saveBlueprintName() {
  ...
  window.api.invoke('saveBlueprint', saveData).then(function(result) {
      if (result.success) {
  ...
  return new Promise((resolve) => {
    if (imageData && imageData.startsWith('data:')) {
      ...
      img.onload = function() {
        ...
        renderDeviceMarkers();
        // Delayed re-sync to catch any layout shift after initial render
        setTimeout(function() {
          syncMarkersToImage();
          renderDeviceMarkers();
        }, 300);
      ...
      BlueprintDB.saveImage('all-devices-map', imageData).then(function() {
        ...
        uploadImageToDrive('all-devices-map', jpegDataUrl)
          .then(function(result) {
            ...
            window.api.invoke('saveBlueprint', { id: 'all-devices-map', name: 'All Devices Map', imageData: 'drive:' + result.driveFileId }).then(function() {
          ...
          .catch(function(err) {
            ...
            window.api.invoke('saveBlueprint', { id: 'all-devices-map', name: 'All Devices Map', imageData: 'local:all-devices-map' }).then(function(){}).catch(function(){});
            updateBlueprintDisplay();
            renderDeviceMarkers();
      ...
      BlueprintDB.saveImage(blueprintId, imageData).then(function(saved) {
        ...
        uploadImageToDrive(blueprintId, jpegDataUrl)
          ...
          .catch(function(err) {
            ...
            window.api.invoke('saveBlueprint', { id: blueprintId, name: bp.name || blueprintId, imageData: 'local:' + blueprintId }).then(function() {}).catch(function() {});
            updateBlueprintDisplay();
            showToast('Cloud upload failed. Map saved locally only.', 'warning');
          });
      });
...
function createNewBlueprint(name) {
  ...
  window.api.invoke('saveBlueprint', newBlueprint).then(function(result) {
      if (result.success) {
...
function deleteCurrentBlueprint() {
  ...
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('deleteBlueprint', state.activeBlueprint).then(function(result) {
          if (result.success) {
...
function syncMarkersToImage() {
  ...
  // Get the NATURAL (un-transformed) rendered size of the image
  // offsetWidth/offsetHeight can be affected by CSS transforms, so use naturalWidth
  // scaled by the CSS max-width/max-height constraints
  var imgWidth = img.offsetWidth;
  var imgHeight = img.offsetHeight;

  // Fallback: if offsetWidth returns 0 (can happen with transforms), compute from natural size
  if (imgWidth <= 0 || imgHeight <= 0) {
    // The image has max-width/max-height constraints, compute the actual rendered size
    var maxW = img.parentElement ? img.parentElement.clientWidth : window.innerWidth - 150;
    var maxH = window.innerHeight - 250;
    var natW = img.naturalWidth;
    var natH = img.naturalHeight;
    var scale = Math.min(maxW / natW, maxH / natH, 1);
    imgWidth = Math.round(natW * scale);
    imgHeight = Math.round(natH * scale);
  }
...
function assignTrapToTech(deviceIp, techName, deviceId) {
  ...
  window.api.invoke('assignTrapsByIp', deviceIp, techName).then(function(result) {
      if (result.success) {
...
function assignServiceRequestToTech(requestId, techName, deviceId) {
  ...
  window.api.invoke('assignServiceRequest', requestId, techName).then(function(result) {
      showToast('Assigned to ' + techName, 'success');
...
function unassignTrap(trapId, deviceId) {
  window.api.invoke('unassignTrap', trapId).then(function(result) {
...
function unassignServiceRequest(requestId, deviceId) {
  ...
  window.api.invoke('unassignServiceRequest', requestId).then(function(result) {
      showToast('Assignment removed', 'success');
      if (result && result.updatedRequest) {
        mergeServerUpdate(result.updatedRequest);
      }
...
function completeServiceRequestDirect(requestId) {
  ...
  window.api.invoke('completeServiceRequest', requestId, 'Completed from device modal').then(function(result) {
...
function resolveAllDeviceAlerts(deviceId, deviceIp) {
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('resolveTrapsByIp', deviceIp, state.currentTechName || 'User').then(function(result) {
          var deviceServiceRequests = (serviceRequestsData || []).filter(function(sr) {
            return (sr.deviceId === deviceId) && (sr.status === 'pending' || sr.status === 'in-progress');
          });
          var promises = deviceServiceRequests.map(function(sr) {
            return new Promise(function(resolve) {
              window.api.invoke('completeServiceRequest', sr.id, 'Bulk resolved').then(resolve).catch(resolve);
            });
          });
...
function sendManufacturerEmailFromModal(deviceId) {
  ...
  window.api.invoke('sendManufacturerEmail', deviceId, email, message, cc, subject).then(function(result) {
      if (result.success) {
...
function previewManufacturerEmail(deviceId) {
  ...
  window.api.invoke('previewManufacturerEmail', deviceId, message).then(function(result) {
      if (result && result.success) {
...
function sendManufacturerEmail(deviceId) {
  ...
  window.api.invoke('sendManufacturerEmail', deviceId, email, message).then(function(result) {
      if (result.success) {
...
function pickerSelectDevice(deviceId) {
  ...
  window.api.invoke('saveDevice', device).then(function(result) {
      if (result.success) {
...
function saveDeviceForm(event) {
  ...
  window.api.invoke('saveDevice', device).then(result => {
      if (result.success) {
        closeAddDeviceModal();
...
function confirmDeleteDevice(deviceId) {
  showConfirmCard({
    ...
    confirmText: 'Delete',
    onConfirm: function() {
      window.api.invoke('deleteDevice', deviceId).then(result => {
          if (result.success) {
            closeDeviceModal();
...
function resolveTrap(trapId) {
  window.api.invoke('resolveTrap', trapId, 'User').then(result => {
      if (result.success) {
...
function createTrapTicket(sourceIp, message, trapId) {
  ...
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('createServiceRequest', {
          deviceId: deviceId,
...
function resolveDeviceTraps(ip) {
  window.api.invoke('resolveTrapsByIp', ip, 'User').then(result => {
      if (result.success) {
...
function clearAllTraps() {
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('clearAllTraps').then(function(result) {
          if (result && result.success) {
            state.traps = [];
            renderTrapsList();
            renderDashboard();
...
function emailDevice(deviceId) {
  ...
  showInputCard({
    ...
    onConfirm: function(customMessage) {
      window.api.invoke('sendDeviceEmail', deviceId, customMessage || '').then(function(result) {
          if (result.success) {
            showToast('Email sent successfully', 'success');
          } else {
            showToast(result.error || 'Error sending email', 'error');
...
function saveEmailSettings(event) {
  ...
  window.api.invoke('saveEmailConfig', config).then(result => {
      if (result.success) {
...
function toggleSmtpPasswordVisibility() {
  var input = document.getElementById('smtp-password');
  var icon = document.getElementById('smtp-password-toggle-icon');
...
function testSmtpConnection() {
  ...
  var resultDiv = document.getElementById('smtp-test-result');

  if (!host) { showToast('Please enter an SMTP host first', 'warning'); return; }
  if (!user || !password) { showToast('Please enter SMTP username and password', 'warning'); return; }

  resultDiv.style.display = 'block';
  resultDiv.innerHTML = '<span style="color:#6b7280;"><i data-lucide="loader" style="width:14px;height:14px;animation:spin 1s linear infinite;display:inline-block;vertical-align:middle;"></i> Testing SMTP connection...</span>';
  if (typeof lucide !== 'undefined') lucide.createIcons();

  // Save first, then test
  saveEmailSettings(new Event('submit'));

  setTimeout(function() {
    window.api.invoke('testSmtpConnection').then(function(result) {
      if (result && result.success) {
        resultDiv.innerHTML = '<span style="color:#16a34a;font-weight:600;">&#10003; SMTP connection successful! Ready to send emails.</span>';
      } else {
        resultDiv.innerHTML = '<span style="color:#dc2626;font-weight:600;">&#10007; Connection failed: ' + ((result && result.error) || 'Unknown error') + '</span>';
      }
    }).catch(function(err) {
      resultDiv.innerHTML = '<span style="color:#dc2626;font-weight:600;">&#10007; Connection failed: ' + err.message + '</span>';
    });
  }, 500);
}
...
function testEmail() {
  ...
  window.api.invoke('sendDeviceEmail', deviceId, 'This is a test email from Smart School Monitor.').then(result => {
      if (result.success) {
...
// ============================================
// Gmail / Google OAuth2 Email
// ============================================
function msGraphLoadStatus() {
  window.api.invoke('msGraphGetStatus').then(function(status) {
    var signedOutDiv = document.getElementById('ms-signed-out');
    var signedInDiv = document.getElementById('ms-signed-in');
    if (!signedOutDiv || !signedInDiv) return;

    if (status && status.signedIn) {
      signedOutDiv.style.display = 'none';
      signedInDiv.style.display = 'block';
      document.getElementById('ms-account-name').textContent = status.accountName || 'Unknown';
      document.getElementById('ms-account-email').textContent = status.accountEmail || '';
      var avatar = document.getElementById('ms-account-avatar');
      if (avatar && status.accountName) {
        avatar.textContent = status.accountName.charAt(0).toUpperCase();
      }
    } else {
      signedOutDiv.style.display = 'block';
      signedInDiv.style.display = 'none';
      // Load saved client ID if any
      if (status && status.clientId) {
        document.getElementById('ms-client-id').value = status.clientId;
      }
    }
    if (typeof lucide !== 'undefined') lucide.createIcons();
  }).catch(function(err) {
...
function msGraphSignIn() {
  var clientId = document.getElementById('ms-client-id').value.trim();
  var clientSecret = document.getElementById('ms-tenant-id').value.trim();
  var statusDiv = document.getElementById('ms-graph-status');

  if (!clientId) {
    showToast('Please enter your Google OAuth Client ID', 'warning');
    return;
  }
  if (!clientSecret) {
    showToast('Please enter your Client Secret', 'warning');
    return;
  }

  statusDiv.style.display = 'block';
  statusDiv.innerHTML = '<span style="color:#6b7280;">Opening Google sign-in window...</span>';

  window.api.invoke('msGraphSignIn', clientId, clientSecret).then(function(result) {
    if (result && result.success) {
      statusDiv.innerHTML = '<span style="color:#16a34a;font-weight:600;">&#10003; Signed in as ' + result.account.name + '</span>';
      showToast('Signed in to Google!', 'success');
      msGraphLoadStatus();
    } else {
      statusDiv.innerHTML = '<span style="color:#dc2626;">&#10007; ' + ((result && result.error) || 'Sign-in failed') + '</span>';
      showToast((result && result.error) || 'Sign-in failed', 'error');
    }
  }).catch(function(err) {
    statusDiv.innerHTML = '<span style="color:#dc2626;">&#10007; ' + err.message + '</span>';
    showToast('Sign-in error: ' + err.message, 'error');
  });
}

function msGraphSignOut() {
  window.api.invoke('msGraphSignOut').then(function() {
...
function msGraphTestSend() {
  var statusDiv = document.getElementById('ms-graph-status');
  statusDiv.style.display = 'block';
  statusDiv.innerHTML = '<span style="color:#6b7280;">Sending test email to yourself...</span>';

  window.api.invoke('msGraphGetStatus').then(function(status) {
    if (!status || !status.accountEmail) {
      statusDiv.innerHTML = '<span style="color:#dc2626;">Not signed in</span>';
      return;
    }
    return window.api.invoke('msGraphSendEmail',
      status.accountEmail,
      'Smart School Monitor - Test Email',
      'This is a test email sent via Gmail API from Smart School Monitor.',
      '<div style="font-family:sans-serif;padding:20px;"><h2 style="color:#4285f4;">&#10003; Email Working!</h2><p>This test email was sent from <strong>Smart School Monitor</strong> via Gmail API.</p><p style="color:#666;font-size:12px;">Sent at ' + new Date().toLocaleString() + '</p></div>',
      {}
    );
  }).then(function(result) {
    if (result && result.success) {
      statusDiv.innerHTML = '<span style="color:#16a34a;font-weight:600;">&#10003; Test email sent! Check your inbox.</span>';
      showToast('Test email sent via Gmail!', 'success');
    } else if (result) {
      statusDiv.innerHTML = '<span style="color:#dc2626;">&#10007; ' + ((result && result.error) || 'Failed') + '</span>';
    }
  }).catch(function(err) {
    statusDiv.innerHTML = '<span style="color:#dc2626;">&#10007; ' + err.message + '</span>';
  });
...
function applySettings() {
  ...
  if (smtpHost) smtpHost.value = state.emailConfig.smtpHost || '';
  var smtpPort = document.getElementById('smtp-port');
  if (smtpPort) smtpPort.value = state.emailConfig.smtpPort || '587';
  var smtpSecure = document.getElementById('smtp-secure');
  if (smtpSecure) smtpSecure.value = state.emailConfig.smtpSecure || 'false';
  var smtpUser = document.getElementById('smtp-user');
  if (smtpUser) smtpUser.value = state.emailConfig.smtpUser || '';
  var smtpPass = document.getElementById('smtp-password');
  if (smtpPass) smtpPass.value = state.emailConfig.smtpPassword || '';

  // Load Microsoft 365 sign-in status
  msGraphLoadStatus();
...
function saveAfterHoursSettings() {
  ...
  // IMPORTANT: Stringify workDays to avoid serialization issues
  var settings = {
  ...
  window.api.invoke('saveAfterHoursSettings', settings).then(function(result) {
      if (result.success) {
...
function loadAfterHoursSettings() {
  window.api.invoke('getAfterHoursSettings').then(function(settings) {
      if (settings) {
...
function confirmClearDevices() {
  showConfirmCard({
    ...
    onConfirm: function() {
      window.api.invoke('clearAllDevices').then(result => {
          if (result.success) {
            state.devices = [];
            updateAllStats();
            renderDeviceMarkers();
...
function exportData() {
  window.api.invoke('exportAllData').then(data => {
...
function assignToMe(deviceIp) {
  function doAssign() {
    window.api.invoke('assignTrapsByIp', deviceIp, state.currentTechName).then(function(result) {
        if (result.success) {
...
function unassignDeviceTraps(deviceIp) {
  window.api.invoke('assignTrapsByIp', deviceIp, '').then(function(result) {
      if (result.success) {
        showToast('Assignment removed', 'success');
        loadTraps().then(function() {
          renderTrapsList();
...
function saveTechnicianFromModal() {
  ...
  window.api.invoke('saveTechnician', {
      name: name.trim(),
      email: email.trim(),
      phone: phone.trim(),
      active: true
...
function deleteTechnicianConfirm(techId) {
  showConfirmCard({
    ...
    confirmText: 'Remove',
    onConfirm: function() {
      window.api.invoke('deleteTechnician', techId).then(function(result) {
          if (result.success) {
...
function saveNewPassword() {
  ...
  window.api.invoke('setSecurityPassword', newPassword, oldPassword).then(function(result) {
      if (result.success) {
...
function removePassword() {
  showConfirmCard({
    ...
    onConfirm: function() {
      window.api.invoke('setSecurityPassword', '', '').then(function(result) {
          if (result.success) {
            state.passwordProtected = false;
            state.isAuthenticated = true;
            sessionStorage.removeItem('authenticated');
...
async function loadTeachers() {
  return new Promise(function(resolve, reject) {
    window.api.invoke('getTeachers').then(function(teachers) {
...
function saveTeacherForm(event) {
  ...
  window.api.invoke('saveTeacher', teacher).then(function(result) {
      if (result.success) {
...
function confirmDeleteTeacher(teacherId) {
  ...
  showConfirmCard({
    ...
    confirmText: 'Delete',
    onConfirm: function() {
      window.api.invoke('deleteTeacher', teacherId).then(function(result) {
          if (result.success) {
...
function confirmCSVImport() {
  ...
  window.api.invoke('importTeachers', pendingCSVData).then(function(result) {
...
function exportTeachersCSV() {
  window.api.invoke('exportTeachers').then(function(result) {
    ...
    onConfirm: function() {
      ...
      showConfirmCard({
        ...
        onConfirm: function() {
          ...
          window.api.invoke('deleteAllTeachers').then(function(result) {
              if (result.success) {
                state.teachers = [];
                renderTeachersTable();
                updateTeachersCount();
...
function loadDeviceTypes() {
  window.api.invoke('getDeviceTypes').then(function(data) {
...
function saveDeviceTypeForm(event) {
  ...
  window.api.invoke('saveDeviceType', data).then(function(result) {
...
function deleteDeviceType(id) {
  ...
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('deleteDeviceType', id).then(function(result) {
...
function loadIssueButtons() {
  window.api.invoke('getIssueButtons').then(function(data) {
...
function openAddIssueButtonModal() {
  ...
  document.getElementById('issue-button-color').value = '#5CB85C';
  document.getElementById('issue-button-color-text').value = '#5CB85C';
  updateIssueButtonIconPreview('alert-circle');
  clearIssueButtonImage();
  populateDeviceTypeSelects();
...
function editIssueButton(id) {
  ...
  document.getElementById('issue-button-order').value = ib.sortOrder || ib.displayOrder || 0;
  document.getElementById('issue-button-active').checked = ib.active !== false;

  // Color
  var btnColor = ib.color || '#5CB85C';
  document.getElementById('issue-button-color').value = btnColor;
  document.getElementById('issue-button-color-text').value = btnColor;

  // Image
  if (ib.imageDataUrl) {
    document.getElementById('issue-button-image-preview-img').src = ib.imageDataUrl;
    document.getElementById('issue-button-image-preview').style.display = 'block';
    document.getElementById('issue-button-image-clear-btn').style.display = '';
    issueButtonImageDataUrl = ib.imageDataUrl;
  } else {
    clearIssueButtonImage();
  }
...
function saveIssueButtonForm(event) {
  ...
  var data = {
    ...
    color: document.getElementById('issue-button-color').value || '#5CB85C',
    ...
    imageDataUrl: issueButtonImageDataUrl || ''
  };
  ...
  window.api.invoke('saveIssueButton', data).then(function(result) {
...
function deleteIssueButton(id) {
  ...
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('deleteIssueButton', id).then(function(result) {
...
// Issue Button Image Upload
var issueButtonImageDataUrl = '';

function uploadIssueButtonImage(event) {
  ...
  var reader = new FileReader();
  reader.onload = function(e) {
    issueButtonImageDataUrl = e.target.result;
    document.getElementById('issue-button-image-preview-img').src = issueButtonImageDataUrl;
    document.getElementById('issue-button-image-preview').style.display = 'block';
    document.getElementById('issue-button-image-clear-btn').style.display = '';
...
function clearIssueButtonImage() {
  issueButtonImageDataUrl = '';
  var preview = document.getElementById('issue-button-image-preview');
  var clearBtn = document.getElementById('issue-button-image-clear-btn');
  if (preview) preview.style.display = 'none';
  if (clearBtn) clearBtn.style.display = 'none';
  var img = document.getElementById('issue-button-image-preview-img');
  if (img) img.src = '';
}

// ═══════════════════════════════════════════
// Request Page Branding Settings
// ═══════════════════════════════════════════
var requestPageBrandingData = null;

function loadRequestPageBranding() {
  window.api.invoke('getRequestPageBranding').then(function(data) {
    requestPageBrandingData = data || {};
    populateBrandingForm(requestPageBrandingData);
...
function populateBrandingForm(b) {
  // Logo
  if (b.logoDataUrl) {
    document.getElementById('branding-logo-preview-img').src = b.logoDataUrl;
    document.getElementById('branding-logo-preview').style.display = 'block';
    document.getElementById('branding-logo-clear-btn').style.display = '';
  } else {
    document.getElementById('branding-logo-preview').style.display = 'none';
    document.getElementById('branding-logo-clear-btn').style.display = 'none';
  }
  // Text fields
  document.getElementById('branding-header-title').value = b.headerTitle || 'SHARKQUICK';
  document.getElementById('branding-header-subtitle').value = b.headerSubtitle || 'SERVICE REQUEST PORTAL';
  document.getElementById('branding-font-family').value = b.fontFamily || 'Inter';
  document.getElementById('branding-button-radius').value = b.buttonBorderRadius || '20';
  document.getElementById('branding-button-radius-val').textContent = b.buttonBorderRadius || '20';

  // New layout fields
  document.getElementById('branding-header-layout').value = b.headerLayout || 'horizontal';
  document.getElementById('branding-logo-size').value = b.logoSize || '140';
  document.getElementById('branding-logo-size-val').textContent = b.logoSize || '140';
  document.getElementById('branding-welcome-text').value = b.welcomeText || 'Welcome, {name}!';
  document.getElementById('branding-show-device-info').value = b.showDeviceInfo || 'minimal';
  document.getElementById('branding-show-remember-username').checked = b.showRememberUsername !== 'false';

  // Colors (original + new)
  var colorFields = [
    { id: 'branding-bg-color', key: 'backgroundColor', def: '#7BA3C9' },
    { id: 'branding-header-bg-color', key: 'headerBackgroundColor', def: '#6B8FB8' },
    { id: 'branding-card-bg-color', key: 'cardBackgroundColor', def: '#F5F0E6' },
    { id: 'branding-device-bg-color', key: 'deviceInfoBackgroundColor', def: '#6B8FB8' },
    { id: 'branding-button-color', key: 'buttonColor', def: '#5CB85C' },
    { id: 'branding-title-color', key: 'titleColor', def: '#3D5A73' },
    { id: 'branding-subtitle-color', key: 'subtitleColor', def: '#C9524A' },
    { id: 'branding-button-text-color', key: 'buttonTextColor', def: '#FFFFFF' },
    // New layout colors
    { id: 'branding-header-stripe-color', key: 'headerStripeColor', def: '#5B8DB8' },
    { id: 'branding-card-gradient-start', key: 'cardGradientStart', def: '#6B8FB8' },
    { id: 'branding-card-gradient-end', key: 'cardGradientEnd', def: '#C8D8E8' },
    { id: 'branding-button-background', key: 'buttonBackground', def: '#F2F2F2' },
    { id: 'branding-employee-card-bg', key: 'employeeCardBackground', def: '#FFFFFF' }
  ];
  colorFields.forEach(function(cf) {
    var val = b[cf.key] || cf.def;
    document.getElementById(cf.id).value = val;
    document.getElementById(cf.id + '-text').value = val;
  });
}

function saveRequestPageBranding() {
  var data = {
    logoDataUrl: (requestPageBrandingData && requestPageBrandingData.logoDataUrl) || '',
    headerTitle: document.getElementById('branding-header-title').value.trim(),
    headerSubtitle: document.getElementById('branding-header-subtitle').value.trim(),
    fontFamily: document.getElementById('branding-font-family').value,
    buttonBorderRadius: document.getElementById('branding-button-radius').value,
    backgroundColor: document.getElementById('branding-bg-color').value,
    headerBackgroundColor: document.getElementById('branding-header-bg-color').value,
    cardBackgroundColor: document.getElementById('branding-card-bg-color').value,
    deviceInfoBackgroundColor: document.getElementById('branding-device-bg-color').value,
    buttonColor: document.getElementById('branding-button-color').value,
    titleColor: document.getElementById('branding-title-color').value,
    subtitleColor: document.getElementById('branding-subtitle-color').value,
    buttonTextColor: document.getElementById('branding-button-text-color').value,
    // New layout fields
    headerLayout: document.getElementById('branding-header-layout').value,
    logoSize: document.getElementById('branding-logo-size').value,
    welcomeText: document.getElementById('branding-welcome-text').value.trim(),
    showDeviceInfo: document.getElementById('branding-show-device-info').value,
    showRememberUsername: document.getElementById('branding-show-remember-username').checked ? 'true' : 'false',
    // New color fields
    headerStripeColor: document.getElementById('branding-header-stripe-color').value,
    cardGradientStart: document.getElementById('branding-card-gradient-start').value,
    cardGradientEnd: document.getElementById('branding-card-gradient-end').value,
    buttonBackground: document.getElementById('branding-button-background').value,
    employeeCardBackground: document.getElementById('branding-employee-card-bg').value
  };

  showToast('Saving branding...', 'info');
  window.api.invoke('saveRequestPageBranding', data).then(function(result) {
    if (result && result.success) {
      requestPageBrandingData = data;
      showToast('Branding saved successfully!', 'success');
    } else {
      showToast('Error saving branding: ' + (result && result.error || 'Unknown error'), 'error');
    }
  }).catch(function(error) {
...
function resetRequestPageBranding() {
  showConfirmCard({
    title: 'Reset Branding',
    message: 'Reset all branding settings to defaults? This will clear the logo and restore default colors.',
    type: 'warning',
    confirmText: 'Reset',
    onConfirm: function() {
      var defaults = {
        logoDataUrl: '', headerTitle: 'SHARKQUICK', headerSubtitle: 'SERVICE REQUEST PORTAL',
      ...
      window.api.invoke('saveRequestPageBranding', defaults).then(function() {
        requestPageBrandingData = defaults;
        populateBrandingForm(defaults);
        updateBrandingPreview();
...
function uploadRequestPageLogo(event) {
  var file = event.target.files && event.target.files[0];
  if (!file) return;
  if (!file.type.startsWith('image/')) {
    showToast('Please upload an image file', 'warning');
  ...
  var reader = new FileReader();
  reader.onload = function(e) {
    if (!requestPageBrandingData) requestPageBrandingData = {};
    requestPageBrandingData.logoDataUrl = e.target.result;
    document.getElementById('branding-logo-preview-img').src = e.target.result;
    document.getElementById('branding-logo-preview').style.display = 'block';
    document.getElementById('branding-logo-clear-btn').style.display = '';
    updateBrandingPreview();
...
function clearRequestPageLogo() {
  if (!requestPageBrandingData) requestPageBrandingData = {};
  requestPageBrandingData.logoDataUrl = '';
  document.getElementById('branding-logo-preview').style.display = 'none';
  document.getElementById('branding-logo-clear-btn').style.display = 'none';
  document.getElementById('branding-logo-preview-img').src = '';
  updateBrandingPreview();
}
...
function updateBrandingPreview() {
  ...
  var bgColor = document.getElementById('branding-bg-color').value;
  var headerBg = document.getElementById('branding-header-bg-color').value;
  var titleColor = document.getElementById('branding-title-color').value;
  var subtitleColor = document.getElementById('branding-subtitle-color').value;
  var btnTextColor = document.getElementById('branding-button-text-color').value;
  var title = document.getElementById('branding-header-title').value || 'SHARKQUICK';
  var subtitle = document.getElementById('branding-header-subtitle').value || 'SERVICE REQUEST PORTAL';
  var font = document.getElementById('branding-font-family').value || 'Inter';
  var radius = document.getElementById('branding-button-radius').value || '20';
  var logoUrl = requestPageBrandingData && requestPageBrandingData.logoDataUrl;

  // New fields
  var headerLayout = document.getElementById('branding-header-layout').value || 'horizontal';
  var stripeColor = document.getElementById('branding-header-stripe-color').value || '#5B8DB8';
  var gradStart = document.getElementById('branding-card-gradient-start').value || '#6B8FB8';
  var gradEnd = document.getElementById('branding-card-gradient-end').value || '#C8D8E8';
  var btnBg = document.getElementById('branding-button-background').value || '#F2F2F2';
  var empBg = document.getElementById('branding-employee-card-bg').value || '#FFFFFF';
  var deviceInfo = document.getElementById('branding-show-device-info').value || 'minimal';
  var welcomeText = document.getElementById('branding-welcome-text').value || 'Welcome, {name}!';
  var showRemember = document.getElementById('branding-show-remember-username').checked;
  var logoSize = document.getElementById('branding-logo-size').value || '140';

  // Scale logo for preview (preview is ~280px, real is ~400px, so scale ~0.3)
  var previewLogoSize = Math.round(parseInt(logoSize) * 0.3);

  // Header: horizontal or vertical layout
  var logoHtml = logoUrl
    ? '<img src="' + logoUrl + '" style="width:' + previewLogoSize + 'px;height:' + previewLogoSize + 'px;border-radius:8px;object-fit:contain;">'
    : '<div style="width:' + previewLogoSize + 'px;height:' + previewLogoSize + 'px;border-radius:8px;background:#ddd;display:flex;align-items:center;justify-content:center;font-size:8px;color:#999;">Logo</div>';

  var headerDirection = headerLayout === 'horizontal' ? 'row' : 'column';
  var headerAlign = headerLayout === 'horizontal' ? 'flex-start' : 'center';
  var textAlign = headerLayout === 'horizontal' ? 'left' : 'center';

  // Device info section
  var deviceHtml = '';
  if (deviceInfo === 'full') {
    deviceHtml = '<div style="font-size:8px;color:#fff;text-align:center;margin-bottom:4px;opacity:0.8;">Device: Printer-101 | Room 204</div>';
  } else if (deviceInfo === 'minimal') {
    deviceHtml = '<div style="font-size:8px;color:#fff;text-align:center;margin-bottom:4px;opacity:0.8;">Printer-101</div>';
  }

  // Welcome text
  var welcomeDisplay = escapeHtml(welcomeText.replace('{name}', 'Alvaro'));

  // Remember username
  var rememberHtml = showRemember
    ? '<div style="display:flex;align-items:center;gap:3px;margin-top:3px;"><div style="width:16px;height:9px;border-radius:5px;background:#5CB85C;"></div><span style="font-size:6px;color:#666;">Remember</span></div>'
    : '';
  ...
  preview.innerHTML =
    // Header with stripe
    '<div style="background:' + headerBg + ';border-radius:14px 14px 0 0;border-bottom:3px solid ' + stripeColor + ';padding:10px 8px;">' +
      '<div style="display:flex;flex-direction:' + headerDirection + ';align-items:center;gap:8px;justify-content:' + headerAlign + ';">' +
        logoHtml +
        '<div style="text-align:' + textAlign + ';">' +
          '<div style="font-size:12px;font-weight:800;color:' + titleColor + ';letter-spacing:1px;">' + escapeHtml(title) + '</div>' +
          '<div style="font-size:7px;font-weight:600;color:' + subtitleColor + ';letter-spacing:0.5px;margin-top:1px;">' + escapeHtml(subtitle) + '</div>' +
        '</div>' +
      '</div>' +
    '</div>' +
    // Content area with gradient
    '<div style="background:linear-gradient(to bottom, ' + gradStart + ', ' + gradEnd + ');border-radius:0 0 14px 14px;padding:8px;">' +
      deviceHtml +
      // Employee input card
      '<div style="background:' + empBg + ';border-radius:10px;padding:6px 8px;margin-bottom:6px;">' +
        '<div style="font-size:7px;font-weight:600;color:#555;margin-bottom:2px;">Employee ID or Name</div>' +
        '<div style="background:#f5f5f5;border:1px solid #ddd;border-radius:6px;padding:3px 5px;font-size:7px;color:#999;">alvaro.rodriguez</div>' +
        rememberHtml +
      '</div>' +
      // Welcome
      '<div style="font-size:9px;font-weight:700;color:#fff;text-align:center;margin-bottom:5px;">' + welcomeDisplay + '</div>' +
      // Buttons grid (3 columns like design)
      '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:3px;">' +
        '<div style="background:' + btnBg + ';border-radius:' + radius + 'px;padding:6px 2px;text-align:center;border:1px solid #e0e0e0;">' +
          '<div style="width:16px;height:16px;margin:0 auto 2px;background:#ddd;border-radius:4px;"></div>' +
          '<div style="font-size:6px;font-weight:600;color:' + btnTextColor + ';">Toner</div>' +
        '</div>' +
        '<div style="background:' + btnBg + ';border-radius:' + radius + 'px;padding:6px 2px;text-align:center;border:1px solid #e0e0e0;">' +
          '<div style="width:16px;height:16px;margin:0 auto 2px;background:#ddd;border-radius:4px;"></div>' +
          '<div style="font-size:6px;font-weight:600;color:' + btnTextColor + ';">Paper Jam</div>' +
        '</div>' +
        '<div style="background:' + btnBg + ';border-radius:' + radius + 'px;padding:6px 2px;text-align:center;border:1px solid #e0e0e0;">' +
          '<div style="width:16px;height:16px;margin:0 auto 2px;background:#ddd;border-radius:4px;"></div>' +
          '<div style="font-size:6px;font-weight:600;color:' + btnTextColor + ';">Staples</div>' +
        '</div>' +
      '</div>' +
    '</div>';
}
...
function loadServiceRequests() {
  ...
  window.api.invoke('getServiceRequests').then(function(data) {
...
function claimAlert(requestId) {
  ...
  function doClaim(name) {
    ...
    window.api.invoke('assignServiceRequest', requestId, name).then(function(result) {
...
function viewServiceRequest(id) {
  ...
  if (emailHistoryEl) {
    ...
    window.api.invoke('getEmailHistory', sr.deviceId || '').then(function(result) {
...
function loadEmailLog() {
  ...
  window.api.invoke('getEmailHistory').then(function(result) {
      if (!result || !result.success || !result.emails || result.emails.length === 0) {
        emailLogData = [];
...
function viewEmailBody(emailId) {
  ...
  window.api.invoke('getEmailById', emailId).then(function(result) {
...
function deleteEmailLogEntry(emailId) {
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('deleteEmailHistoryRecord', emailId).then(function(result) {
          if (result && result.success) {
...
function deleteAllEmailLog() {
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('deleteAllEmailHistory').then(function(result) {
          if (result && result.success) {
...
function assignServiceRequestFromModal() {
  ...
  window.api.invoke('assignServiceRequest', currentServiceRequestId, technician).then(function(result) {
...
function completeServiceRequestFromModal() {
  ...
  window.api.invoke('completeServiceRequest', currentServiceRequestId, notes).then(function(result) {
...
function deleteServiceRequest(id) {
  ...
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('deleteServiceRequest', id).then(function(result) {
    ...
    onConfirm: function() {
      ...
      showConfirmCard({
        ...
        onConfirm: function() {
          ...
          window.api.invoke('deleteAllServiceRequests').then(function(result) {
              if (result.success) {
...
function loadLabelLayout() {
  window.api.invoke('getLabelLayout').then(function(layout) {
      if (!layout) {
...
// Generate a tiny inline SVG QR-like pattern for label preview (no external dependency)
function getPreviewQRImageUrl() {
  // Use server-side QR image endpoint for preview — always reliable
  return '/qr/image/preview-sample.png';
}
...
function saveLabelLayoutSettings() {
  ...
  window.api.invoke('saveLabelLayout', layoutJson).then(function(result) {
      if (result && result.success) {
...
function printSingleLabel(deviceId) {
  ...
  // Use server-side QR image endpoint — always reliable
  var qrImageUrl = '/qr/image/' + deviceId + '.png';
...
function printAllLabels() {
  ...
  // Use server-side QR image endpoint — no async IPC needed, always reliable
  var labelData = state.devices.map(function(device) {
    return { device: device, qrUrl: '/qr/image/' + device.id + '.png' };
  });
...
function generateQRForDevice(deviceId) {
  ...
  // Ensure a QR record exists in the database, then display using server-side image endpoint
  window.api.invoke('generateQRCode', deviceId).then(function(result) {
      displayQRCode(result, device);
    }).catch(function(error) {
      // Even if IPC fails, we can still show the QR using the server endpoint
      displayQRCode({ success: true, qrCode: { qrData: '' } }, device);
    });
}
...
function displayQRCode(result, device) {
  ...
  // Use server-side QR image endpoint — this is 100% reliable (runs in Node.js, not Electron)
  var qrImageSrc = '/qr/image/' + device.id + '.png';
  var qrUrl = (result && result.qrCode && result.qrCode.qrData) ? result.qrCode.qrData : '';

  qrDisplay.innerHTML = '<img src="' + qrImageSrc + '" alt="QR Code" class="qr-code-image" style="width:300px;height:300px;">';
  qrDeviceName.textContent = device.name || 'Unknown Device';
  qrDeviceLocation.textContent = device.location || 'No location';

  // Show the encoded URL prominently so user can verify
  var urlDisplay = document.getElementById('qr-url-display');
  if (urlDisplay) {
    urlDisplay.textContent = qrUrl || ('Server-generated QR for device: ' + device.id);
    urlDisplay.style.fontSize = '13px';
    urlDisplay.style.color = '#0f766e';
    urlDisplay.style.fontWeight = '500';
...
function downloadQRCode() {
  ...
  // Fetch the QR image from the server endpoint and download as a file
  fetch('/qr/image/' + currentQRDeviceId + '.png')
    ...
    .catch(function() { showToast('Failed to download QR code', 'error'); });
}
...
function loadQRCodesTable() {
  ...
  var html = filtered.map(function(device) {
    // Look up device type name from deviceTypesData instead of showing raw UUID
    var typeName = device.type || '-';
    if (deviceTypesData && deviceTypesData.length > 0) {
      var dtMatch = deviceTypesData.find(function(dt) {
        return dt.id === device.type || dt.name.toLowerCase() === (device.type || '').toLowerCase();
      });
      if (dtMatch) typeName = dtMatch.name;
    }
    return '<tr>' +
      ...
      '<td class="actions-cell">' +
        ...
        '</button>' +
        '<button class="btn btn-sm btn-ghost" onclick="editQRDevice(\'' + device.id + '\')" title="Edit Device">' +
          '<i data-lucide="pencil"></i>' +
        '</button>' +
        '<button class="btn btn-sm btn-secondary" onclick="printSingleLabel(\'' + device.id + '\')" title="Print Label">' +
          '<i data-lucide="printer"></i>' +
        '</button>' +
        '<button class="btn btn-sm btn-ghost" onclick="deleteQRDevice(\'' + device.id + '\')" title="Delete Device" style="color:var(--danger,#ef4444)">' +
          '<i data-lucide="trash-2"></i>' +
        '</button>' +
      '</td>' +
    '</tr>';
...
function generateAllQRCodes() {
  ...
  var promises = state.devices.map(function(device) {
    return new Promise(function(resolve) {
      window.api.invoke('generateQRCode', device.id).then(resolve).catch(resolve);
    });
  });
...
function updateAllQRCodeUrls() {
  ...
  window.api.invoke('updateAllQRCodeUrls').then(function(result) {
      if (result && result.success) {
        if (result.updated === 0) {
          showToast('All QR code URLs are already up to date!', 'success');
        } else {
...
// ---- QR Redirect URL (iOS Safari trampoline) ----

function loadQRRedirectUrl() {
  Promise.all([
    window.api.invoke('getEmailConfig'),
    window.api.invoke('getServerNetworkInfo')
  ]).then(function(results) {
    var config = results[0] || {};
    var netInfo = results[1] || {};
    var input = document.getElementById('qr-redirect-url');
    var status = document.getElementById('qr-redirect-status');
    if (!input) return;
    var url = (config.qrRedirectUrl || '').trim();
    input.value = url;
    if (status) {
      var lanInfo = 'LAN IP: <strong>' + (netInfo.ip || '?') + ':' + (netInfo.port || '3847') + '</strong>';
      if (url) {
        status.innerHTML = '<span style="color:var(--success,#16a34a);font-weight:600;">✓ Redirect active</span> — QR codes use trampoline URL. ' + lanInfo;
      } else {
        status.innerHTML = '<span style="color:var(--warning,#d97706);">⚠ Not configured</span> — iPhones may not be able to scan QR codes. ' + lanInfo;
      }
    }
  }).catch(function() {});
}

function saveQRRedirectUrl() {
  var input = document.getElementById('qr-redirect-url');
  if (!input) return;
  ...
  window.api.invoke('getEmailConfig').then(function(config) {
    config.qrRedirectUrl = url;
    return window.api.invoke('saveEmailConfig', config);
  }).then(function() {
    showToast(url ? 'QR redirect URL saved!' : 'QR redirect URL cleared', 'success');
    // Regenerate all QR code URLs with the new redirect
    return window.api.invoke('updateAllQRCodeUrls');
  }).then(function(result) {
    if (result && result.success && result.updated > 0) {
      showToast('Updated ' + result.updated + ' QR code URLs', 'info');
    }
    loadQRCodesTable();
    loadQRRedirectUrl();
  }).catch(function(err) {
    showToast('Error saving redirect URL: ' + err.message, 'error');
  });
}

// ---- QR URL Strategy ----

function loadQRStrategy() {
  window.api.invoke('getEmailConfig').then(function(config) {
    var strategy = (config.qrUrlStrategy || '').trim() || 'direct-ip';
    var select = document.getElementById('qr-url-strategy');
    if (select) select.value = strategy;
    updateQRStrategyPreview();
  }).catch(function() {});
}

function onQRStrategyChange() {
  updateQRStrategyPreview();
  saveQRStrategy();
}

function updateQRStrategyPreview() {
  var select = document.getElementById('qr-url-strategy');
  var previewEl = document.getElementById('qr-strategy-preview');
  if (!select || !previewEl) return;
  var strategy = select.value;

  window.api.invoke('getServerNetworkInfo').then(function(netInfo) {
    var ip = netInfo.ip || '10.x.x.x';
    var port = netInfo.port || '3847';
    var hostname = netInfo.hostname || 'hostname.local';
    var preview = '';

    switch (strategy) {
      case 'cloud-relay':
        window.api.invoke('getEmailConfig').then(function(cfg) {
          var relayUrl = (cfg.cloudRelayUrl || '').trim();
          if (relayUrl) {
            previewEl.textContent = relayUrl + '/request?device=DEVICE_ID';
          } else {
            previewEl.textContent = 'Configure Cloud Relay URL below first';
          }
        });
        return;
      case 'cloudflare-tunnel':
        // Get the actual tunnel URL from config
        window.api.invoke('getEmailConfig').then(function(cfg) {
          var tunnelUrl = (cfg.webAppUrl || '').trim();
          if (tunnelUrl && tunnelUrl.includes('trycloudflare.com')) {
            previewEl.textContent = tunnelUrl + '/request?device=DEVICE_ID';
          } else {
            previewEl.textContent = 'Waiting for tunnel to connect...';
          }
        });
        return;
      case 'gas-trampoline':
        preview = 'https://script.google.com/.../exec?d=DEVICE_ID&s=' + ip + ':' + port;
        break;
      case 'direct-ip':
        preview = 'http://' + ip + ':' + port + '/request?device=DEVICE_ID';
        break;
      case 'ip-trailing-dot':
        preview = 'http://' + ip + '.:' + port + '/request?device=DEVICE_ID';
        break;
      case 'ip-userinfo':
        preview = 'http://s@' + ip + ':' + port + '/request?device=DEVICE_ID';
        break;
      case 'mdns-local':
        preview = 'http://' + hostname + ':' + port + '/request?device=DEVICE_ID';
        break;
    }
    previewEl.textContent = preview;
  }).catch(function() {
    previewEl.textContent = '(Unable to get network info)';
  });
}
...
function loadTunnelStatus() {
  var indicator = document.getElementById('tunnel-indicator');
  var display = document.getElementById('tunnel-url-display');
  if (!indicator || !display) return;

  window.api.invoke('getEmailConfig').then(function(config) {
    var url = (config.webAppUrl || '').trim();
    if (url && url.includes('trycloudflare.com')) {
      indicator.style.background = '#22c55e';
      display.innerHTML = '<span style="color:#22c55e;font-weight:600;">Connected</span> — ' + url;
    } else if (url) {
      indicator.style.background = '#38bdf8';
      display.innerHTML = '<span style="color:#38bdf8;font-weight:600;">Custom URL</span> — ' + url;
    } else {
      indicator.style.background = '#64748b';
      display.textContent = 'Tunnel not connected yet. It starts automatically when the app launches.';
    }
  }).catch(function() {
    indicator.style.background = '#ef4444';
    display.textContent = 'Could not check tunnel status';
  });
}

// Listen for live tunnel URL changes from main process
if (window.api && window.api.on) {
  window.api.on('tunnel-url-changed', function(url) {
    ...
    // Refresh strategy dropdown + preview
    loadQRStrategy();
    updateQRStrategyPreview();
    // Refresh QR codes table
    if (typeof loadQRCodesTable === 'function') loadQRCodesTable();
    showToast('Cloudflare Tunnel connected: ' + url, 'success');
  });
}

// ---- Cloud Relay Settings ----

function loadCloudRelaySettings() {
  var urlInput = document.getElementById('cloud-relay-url');
  var secretInput = document.getElementById('cloud-relay-secret');
  var statusEl = document.getElementById('cloud-relay-status');
  var settingsBox = document.getElementById('cloud-relay-settings');
  if (!urlInput) return;

  window.api.invoke('getEmailConfig').then(function(config) {
    urlInput.value = config.cloudRelayUrl || '';
    secretInput.value = config.cloudRelaySyncSecret || '';

    // Check relay status if URL is configured
    var relayUrl = (config.cloudRelayUrl || '').trim();
    if (relayUrl) {
      statusEl.textContent = 'Checking relay...';
      fetch(relayUrl + '/api/sync/status', { mode: 'cors' })
        .then(function(r) { return r.json(); })
        .then(function(data) {
          if (data.online) {
            var syncIndicator = document.getElementById('cloud-sync-indicator');
            if (syncIndicator) syncIndicator.style.background = '#22c55e';
            var info = data.deviceCount + ' devices, ' + data.teacherCount + ' teachers';
            if (data.lastSyncAt) {
              var ago = Math.round((Date.now() - new Date(data.lastSyncAt).getTime()) / 1000);
              info += ' — synced ' + (ago < 60 ? ago + 's ago' : Math.round(ago / 60) + 'm ago');
            }
            statusEl.innerHTML = '<span style="color:#22c55e;">Online</span> — ' + info;
          }
        })
        .catch(function() {
          statusEl.innerHTML = '<span style="color:#ef4444;">Offline</span> — cannot reach relay';
          var syncIndicator = document.getElementById('cloud-sync-indicator');
          if (syncIndicator) syncIndicator.style.background = '#ef4444';
        });
    } else {
      statusEl.textContent = 'Not configured. Deploy cloud-relay/ to Render.com and enter the URL above.';
    }
  });

  // Show/hide based on selected strategy
  var select = document.getElementById('qr-url-strategy');
  if (settingsBox && select) {
    settingsBox.style.display = (select.value === 'cloud-relay') ? 'block' : 'block';
  }
}

function saveCloudRelaySettings() {
  var urlInput = document.getElementById('cloud-relay-url');
  var secretInput = document.getElementById('cloud-relay-secret');
  if (!urlInput) return;

  window.api.invoke('getEmailConfig').then(function(config) {
    config.cloudRelayUrl = (urlInput.value || '').trim();
    config.cloudRelaySyncSecret = (secretInput.value || '').trim();
    return window.api.invoke('saveEmailConfig', config);
  }).then(function() {
    showToast('Cloud relay settings saved', 'success');
    loadCloudRelaySettings();
    updateQRStrategyPreview();
  }).catch(function(err) {
    showToast('Error saving relay settings: ' + err.message, 'error');
  });
}

// Listen for cloud sync status updates from main process
if (window.api && window.api.on) {
  window.api.on('cloud-sync-status', function(status) {
    var syncIndicator = document.getElementById('cloud-sync-indicator');
    var statusEl = document.getElementById('cloud-relay-status');
    if (syncIndicator) {
      syncIndicator.style.background = status.ok ? '#22c55e' : '#ef4444';
    }
    if (statusEl && status.ok && status.counts) {
      statusEl.innerHTML = '<span style="color:#22c55e;">Synced</span> — ' +
        (status.counts.devices || 0) + ' devices, ' +
        (status.counts.teachers || 0) + ' teachers';
    }
  });
  window.api.on('cloud-sync-new-requests', function(count) {
    showToast(count + ' new service request(s) synced from cloud relay', 'info');
    // Refresh service requests if visible
    if (typeof loadServiceRequests === 'function') loadServiceRequests();
  });
}

function saveQRStrategy() {
  var select = document.getElementById('qr-url-strategy');
  if (!select) return;
  var strategy = select.value;
  ...
  window.api.invoke('getEmailConfig').then(function(config) {
    ...
    var msg = 'QR strategy saved: ' + select.options[select.selectedIndex].text;
    if (result && result.updated > 0) {
      msg += ' (' + result.updated + ' QR codes updated)';
    }
    showToast(msg, 'success');
    var statusEl = document.getElementById('qr-strategy-status');
    if (statusEl) statusEl.innerHTML = '<span style="color:var(--success,#16a34a);">✓ Strategy saved</span>';
    loadQRCodesTable();
  }).catch(function(err) {
...
function openTestConnection() {
  // Open the test-connection page — prefer tunnel URL if available
  Promise.all([
    window.api.invoke('getServerNetworkInfo'),
    window.api.invoke('getEmailConfig')
  ]).then(function(results) {
    ...
    var ip = netInfo.ip || 'localhost';
    var port = netInfo.port || '3847';
    var tunnelUrl = (config.webAppUrl || '').trim();

    // Prefer tunnel URL (works on any phone), fall back to local
    var url;
    if (tunnelUrl && tunnelUrl.includes('trycloudflare.com')) {
      url = tunnelUrl + '/test-connection';
    } else {
      var host = netInfo.hostname || (ip + '.');
      url = 'http://' + host + ':' + port + '/test-connection';
    }

    var qrImgUrl = 'http://localhost:' + port + '/qr/test-connection.png';
    showConfirmCard({
      title: 'Test QR Strategies from Phone',
      message: '<div style="text-align:center;"><img src="' + qrImgUrl + '" style="width:200px;height:200px;border-radius:8px;margin:8px auto;display:block;background:#fff;padding:4px;" alt="QR Code"><p style="margin-top:8px;font-size:0.8rem;color:var(--text-muted,#64748b);word-break:break-all;">' + url + '</p><p style="margin-top:8px;font-size:0.75rem;color:var(--text-muted,#94a3b8);">Scan this QR code from your iPhone to test which strategies work on the school network.</p></div>',
      type: 'info',
    ...
    showToast('Could not get server network info', 'error');
  });
}

// ---- QR Device Management: Edit, Delete, Import ----

function editQRDevice(deviceId) {
  var device = state.devices.find(function(d) { return d.id === deviceId; });
  if (!device) { showToast('Device not found', 'error'); return; }
  ...
  var typeName = device.type || '';
  if (deviceTypesData && deviceTypesData.length > 0) {
    var dtMatch = deviceTypesData.find(function(dt) { return dt.id === device.type || dt.name.toLowerCase() === (device.type || '').toLowerCase(); });
    if (dtMatch) typeName = dtMatch.name;
  }
  ...
  if (deviceTypesData) {
    deviceTypesData.forEach(function(dt) {
      var sel = (dt.id === device.type || dt.name.toLowerCase() === (device.type || '').toLowerCase()) ? ' selected' : '';
      typeOptions += '<option value="' + dt.id + '"' + sel + '>' + escapeHtml(dt.name) + '</option>';
    });
  }

  var modalHtml = '<div class="modal-overlay active" id="edit-qr-device-modal" onclick="if(event.target===this)this.remove()">' +
    '<div class="modal-container" style="max-width:480px">' +
      '<div class="modal-header"><h3>Edit Device</h3><button class="modal-close" onclick="document.getElementById(\'edit-qr-device-modal\').remove()">&times;</button></div>' +
      '<div class="modal-body">' +
        '<div class="form-group"><label class="form-label">Device Name</label><input type="text" class="form-control" id="edit-qr-name" value="' + escapeHtml(device.name || '') + '"></div>' +
        '<div class="form-group"><label class="form-label">Machine ID</label><input type="text" class="form-control" id="edit-qr-machineId" value="' + escapeHtml(device.machineId || '') + '"></div>' +
        '<div class="form-group"><label class="form-label">Location</label><input type="text" class="form-control" id="edit-qr-location" value="' + escapeHtml(device.location || '') + '"></div>' +
        '<div class="form-group"><label class="form-label">Serial Number</label><input type="text" class="form-control" id="edit-qr-serial" value="' + escapeHtml(device.serialNumber || '') + '"></div>' +
        '<div class="form-group"><label class="form-label">Model</label><input type="text" class="form-control" id="edit-qr-model" value="' + escapeHtml(device.model || '') + '"></div>' +
        '<div class="form-group"><label class="form-label">IP Address</label><input type="text" class="form-control" id="edit-qr-ip" value="' + escapeHtml(device.ip || '') + '"></div>' +
        '<div class="form-group"><label class="form-label">Device Type</label><select class="form-control" id="edit-qr-type">' + typeOptions + '</select></div>' +
      '</div>' +
      '<div class="modal-footer">' +
        '<button class="btn btn-ghost" onclick="document.getElementById(\'edit-qr-device-modal\').remove()">Cancel</button>' +
        '<button class="btn btn-primary" onclick="saveQRDevice(\'' + deviceId + '\')">Save Changes</button>' +
      '</div>' +
    '</div></div>';
...
function saveQRDevice(deviceId) {
  var updates = {
    name: document.getElementById('edit-qr-name').value.trim(),
    machineId: document.getElementById('edit-qr-machineId').value.trim(),
    location: document.getElementById('edit-qr-location').value.trim(),
    serialNumber: document.getElementById('edit-qr-serial').value.trim(),
    model: document.getElementById('edit-qr-model').value.trim(),
    ip: document.getElementById('edit-qr-ip').value.trim(),
    type: document.getElementById('edit-qr-type').value
  };

  window.api.invoke('saveDevice', Object.assign({ id: deviceId }, updates)).then(function(result) {
    if (result && result.success !== false) {
      // Update local state
      var idx = state.devices.findIndex(function(d) { return d.id === deviceId; });
      if (idx !== -1) Object.assign(state.devices[idx], updates);
      showToast('Device updated', 'success');
      loadQRCodesTable();
      var modal = document.getElementById('edit-qr-device-modal');
      if (modal) modal.remove();
    } else {
...
function deleteQRDevice(deviceId) {
  var device = state.devices.find(function(d) { return d.id === deviceId; });
  var name = device ? device.name || device.machineId || deviceId : deviceId;
  if (!confirm('Delete device "' + name + '"?\n\nThis will remove the device and its QR code. This cannot be undone.')) return;

  window.api.invoke('deleteDevice', deviceId).then(function(result) {
    if (result && result.success !== false) {
      state.devices = state.devices.filter(function(d) { return d.id !== deviceId; });
      showToast('Device deleted', 'success');
      loadQRCodesTable();
      // Also refresh map markers
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(ev) {
      try {
        ...
        if (lines.length < 2) { showToast('CSV file must have a header row and at least one data row', 'error'); return; }

        var headers = lines[0].split(',').map(function(h) { return h.trim().toLowerCase().replace(/['"]/g, ''); });
        var nameIdx = headers.indexOf('name');
        var machineIdIdx = headers.indexOf('machineid') !== -1 ? headers.indexOf('machineid') : headers.indexOf('machine id');
        var locationIdx = headers.indexOf('location');
        var serialIdx = headers.indexOf('serialnumber') !== -1 ? headers.indexOf('serialnumber') : headers.indexOf('serial number');
        if (serialIdx === -1) serialIdx = headers.indexOf('serial');
        var modelIdx = headers.indexOf('model');
        var ipIdx = headers.indexOf('ip') !== -1 ? headers.indexOf('ip') : headers.indexOf('ip address');
        ...
        for (var i = 1; i < lines.length; i++) {
          ...
          var deviceData = {
            ...
            machineId: machineIdIdx >= 0 ? cols[machineIdIdx] || '' : '',
          ...
          // Must have at least a name or machineId
          if (!deviceData.name && !deviceData.machineId) { errors++; processed++; continue; }

          (function(dd) {
            window.api.invoke('saveDevice', dd).then(function(result) {
              processed++;
              if (result && result.success) {
                imported++;
              } else { errors++; }
              if (processed >= total) {
                showToast('Imported ' + imported + ' devices' + (errors > 0 ? ' (' + errors + ' errors)' : ''), imported > 0 ? 'success' : 'error');
                loadQRCodesTable();
                // Refresh devices from backend to get accurate data
                window.api.invoke('getDevices').then(function(devices) { state.devices = devices || []; loadQRCodesTable(); });
              }
            }).catch(function() { processed++; errors++; if (processed >= total) { showToast('Imported ' + imported + ' devices (' + errors + ' errors)', imported > 0 ? 'success' : 'error'); loadQRCodesTable(); } });
          })(deviceData);
        }
      } catch (err) {
...
function loadEmailTemplates() {
  window.api.invoke('getEmailTemplates').then(function(templates) {
...
function saveEmailTemplate() {
  ...
  window.api.invoke('saveEmailTemplate', template).then(function(result) {
      if (result.success) {
...
function deleteEmailTemplate(templateId) {
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('deleteEmailTemplate', templateId).then(function(result) {
          if (result.success) {
...
function loadHelpdeskSettings() {
  window.api.invoke('getSettings').then(function(settings) {
      if (settings) {
...
function saveHelpdeskSiteNumber() {
  ...
  window.api.invoke('saveSetting', 'servicenowSiteNumber', val).then(function() {
      var status = document.getElementById('helpdesk-site-status');
...
function runAiClassification(text) {
  ...
  window.api.invoke('classifyIncident', text, empName, roomNumber).then(function(result) {
...
function lookupHelpdeskEmployee() {
  ...
  window.api.invoke('lookupEmployee', empId).then(function(result) {
...
function pollForIncNumber() {
  ...
  window.api.invoke('getLatestSnIncident', snLookupEmpId, snLookupShortDesc).then(function(result) {
...
function saveIncident(sendNow) {
  ...
  window.api.invoke('createIncident', data).then(function(result) {
      if (result && result.success) {
        ...
        if (sendNow) {
          showToast('Sending email...', 'info');
          window.api.invoke('sendIncidentEmail', incidentId).then(function(emailResult) {
          ...
          window.api.invoke('queueIncidentEmail', incidentId).then(function(queueResult) {
...
function saveAiTrainingData(incidentId, data) {
  ...
  window.api.invoke('saveTrainingEntry', trainingData).then(function() {
...
function loadIncidents() {
  window.api.invoke('getIncidents', 50).then(function(result) {
...
function loadEmailQueueTable() {
  window.api.invoke('getEmailQueue').then(function(result) {
...
function processEmailQueueNow() {
  ...
  window.api.invoke('processEmailQueue').then(function(result) {
      if (result && result.success) {
...
// ── CodeMAPCopier Import Functions ──

function refreshImportStatus() {
  window.api.invoke('getImportStatus').then(function(status) {
    document.getElementById('import-device-count').textContent = status.currentDevices;
    document.getElementById('import-teacher-count').textContent = status.currentTeachers;
    document.getElementById('import-device-source').textContent = 'Source: ' + status.sourceDevices + ' available';
    document.getElementById('import-teacher-source').textContent = 'Source: ' + status.sourceTeachers + ' available';

    var statusEl = document.getElementById('import-source-status');
    if (status.codeMAPExists) {
      statusEl.innerHTML = '<span style="color: var(--success, #10b981);">CodeMAPCopier directory found</span> &mdash; ' +
        status.sourceDevices + ' devices and ' + status.sourceTeachers + ' employees available for import.';
    } else {
      statusEl.innerHTML = '<span style="color: var(--warning, #f59e0b);">CodeMAPCopier directory not found.</span> ' +
        'Expected at the sibling directory to this app.';
    }
  }).catch(function(err) {
    console.error('Failed to get import status:', err);
    document.getElementById('import-source-status').textContent = 'Error checking import status: ' + err.message;
  });
}

function runCodeMAPImport() {
  var btn = document.getElementById('btn-run-import');
  var resultInfo = document.getElementById('import-result-info');
  var resultText = document.getElementById('import-result-text');

  var options = {
    importDevices: document.getElementById('import-opt-devices').checked,
    importTeachers: document.getElementById('import-opt-teachers').checked,
    importConfig: document.getElementById('import-opt-config').checked,
    importIconPositions: document.getElementById('import-opt-icons').checked,
    forceOverwrite: document.getElementById('import-opt-overwrite').checked
  };

  // Confirm overwrite
  if (options.forceOverwrite) {
    if (!confirm('This will DELETE all existing data before importing. Are you sure?')) {
      return;
    }
  }
  ...
  window.api.invoke('importFromCodeMAP', options).then(function(result) {
    btn.disabled = false;
    btn.innerHTML = '<i data-lucide="download"></i> Run Import';
    if (typeof lucide !== 'undefined') lucide.createIcons();

    resultInfo.style.display = 'flex';

    var parts = [];
    if (result.devices !== undefined) parts.push(result.devices + ' devices');
    if (result.teachers !== undefined) parts.push(result.teachers + ' teachers');
    if (result.config) parts.push('gateway config');
    if (result.iconPositions !== undefined && result.iconPositions > 0) parts.push(result.iconPositions + ' icon positions');

    if (result.success) {
      resultInfo.style.color = 'var(--success, #10b981)';
      resultText.textContent = 'Import successful: ' + parts.join(', ');
    } else {
      resultInfo.style.color = 'var(--warning, #f59e0b)';
      resultText.textContent = 'Imported ' + parts.join(', ') + '. Warnings: ' + result.errors.join('; ');
    }

    // Refresh the status counts
    refreshImportStatus();

    // Refresh relevant data in the app
    if (result.devices > 0 && typeof loadDevices === 'function') loadDevices();
    if (result.teachers > 0 && typeof loadTeachers === 'function') loadTeachers();
  }).catch(function(err) {
    btn.disabled = false;
    btn.innerHTML = '<i data-lucide="download"></i> Run Import';
    if (typeof lucide !== 'undefined') lucide.createIcons();
    ...
    resultInfo.style.display = 'flex';
    resultInfo.style.color = 'var(--danger, #ef4444)';
    resultText.textContent = 'Import failed: ' + err.message;
  });
}

function fetchPositionsFromGoogleSheet() {
  var btn = document.getElementById('btn-fetch-positions');
  var resultDiv = document.getElementById('fetch-positions-result');
  var resultText = document.getElementById('fetch-positions-text');

  btn.disabled = true;
  btn.innerHTML = '<i data-lucide="loader-2" class="spin"></i> Fetching from Google Sheet...';
  if (typeof lucide !== 'undefined') lucide.createIcons();
  resultDiv.style.display = 'none';

  window.api.invoke('importPositionsFromGoogleSheet').then(function(result) {
    btn.disabled = false;
    btn.innerHTML = '<i data-lucide="map-pin"></i> Fetch Icon Positions from Google Sheet';
    if (typeof lucide !== 'undefined') lucide.createIcons();
    ...
    if (result.success) {
      var parts = [];
      if (result.devicesUpdated > 0) parts.push(result.devicesUpdated + ' device positions updated');
      if (result.devicesInserted > 0) parts.push(result.devicesInserted + ' new devices added');
      if (result.blueprintsImported > 0) parts.push(result.blueprintsImported + ' blueprints imported');
      if (result.blueprintImagesDownloaded > 0) parts.push(result.blueprintImagesDownloaded + ' map images downloaded');
      if (parts.length === 0) parts.push('No changes needed');

      resultDiv.style.color = 'var(--success, #10b981)';
      resultText.textContent = parts.join(', ');

      // Refresh device views
      if (typeof loadDevices === 'function') loadDevices();
      if (typeof updateBlueprintDisplay === 'function') updateBlueprintDisplay();
      refreshImportStatus();
    } else {
      resultDiv.style.color = 'var(--danger, #ef4444)';
      resultText.textContent = result.error || result.errors.join('; ');
    }
  }).catch(function(err) {
    btn.disabled = false;
    btn.innerHTML = '<i data-lucide="map-pin"></i> Fetch Icon Positions from Google Sheet';
...
function fetchRepairTemplatesFromGoogleSheet() {
  var btn = document.getElementById('btn-fetch-templates');
  var resultDiv = document.getElementById('fetch-templates-result');
  var resultText = document.getElementById('fetch-templates-text');

  btn.disabled = true;
  btn.innerHTML = '<i data-lucide="loader-2" class="spin"></i> Fetching Repair Templates...';
  if (typeof lucide !== 'undefined') lucide.createIcons();
  resultDiv.style.display = 'none';

  window.api.invoke('importRepairTemplatesFromGoogleSheet').then(function(result) {
    btn.disabled = false;
    btn.innerHTML = '<i data-lucide="clipboard-list"></i> Fetch Repair Templates';
    if (typeof lucide !== 'undefined') lucide.createIcons();
    ...
    if (result.success) {
      resultDiv.style.color = 'var(--success, #10b981)';
      resultText.textContent = result.templatesImported + ' repair templates imported successfully';

      // Refresh the templates list if visible
      if (typeof renderTemplateSettingsList === 'function') renderTemplateSettingsList();
    } else {
      resultDiv.style.color = 'var(--danger, #ef4444)';
      resultText.textContent = result.error || result.errors.join('; ');
    }
...
function loadSheetStats() {
  ...
  window.api.invoke('getSheetStats').then(function(result) {
      if (!result || !result.success) {
        if (tableBody) {
...
function compactDatabase() {
  ...
  window.api.invoke('compactAllSheets').then(function(result) {
...
function openGoogleSheet(sheetName) {
  ...
  window.api.invoke('getSpreadsheetInfo', sheetName).then(function(result) {
...
function createFullBackup() {
  ...
  window.api.invoke('createFullBackup').then(function(result) {
      if (!result || !result.success) {
...
function loadSnmpTrapsTable() {
  ...
  window.api.invoke('getTraps', 200).then(function(traps) {
    ...
    }); // Get more traps for the settings view
}
...
function reprocessTrapsFromUI() {
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('reprocessAllTraps').then(function(result) {
          if (result.success) {
...
function openSnmpTrapsSheet() {
  window.api.invoke('getSpreadsheetInfo', 'SNMPTraps').then(function(result) {
      if (result && result.spreadsheetId) {
        var url = 'https://docs.google.com/spreadsheets/d/' + result.spreadsheetId + '/edit#gid=' + (result.sheetId || '0');
        window.open(url, '_blank');
...
function exportSheetCSV(sheetName) {
  ...
  window.api.invoke('exportSheetAsCSV', sheetName).then(function(result) {
      if (!result || !result.success) {
...
function submitCsvImport() {
  ...
  reader.onload = function(e) {
    ...
    window.api.invoke('importSheetFromCSV', csvImportTargetSheet, csvData).then(function(result) {
...
function clearSheetData(sheetName, rowCount) {
  showConfirmCard({
    ...
    onConfirm: function() {
      ...
      window.api.invoke('clearSheet', sheetName).then(function(result) {
          if (!result || !result.success) {
...
function submitDeviceImport() {
  ...
  reader.onload = function(e) {
    var csvData = e.target.result;
    window.api.invoke('importDevicesFromCSV', csvData).then(function(result) {
        if (btn) btn.disabled = false;
        if (!result || !result.success) {
          if (statusEl) {
            statusEl.style.background = '#fef2f2';
...
function loadAnalytics() {
  ...
  window.api.invoke('getAnalyticsData').then(function(result) {
      if (!result || !result.success) {
        chartIds.forEach(function(id) {
          var el = document.getElementById(id);
...
// ============================================
var preferredCameraId = localStorage.getItem('preferredCameraId') || '';

/**
 * Get video constraints using the preferred USB/external camera.
 * Falls back to any available camera if preferred not found.
 */
function getPreferredVideoConstraints() {
  if (preferredCameraId) {
    return { deviceId: { exact: preferredCameraId }, width: { ideal: 1280 }, height: { ideal: 720 } };
  }
  return { width: { ideal: 1280 }, height: { ideal: 720 } };
}

/**
 * Enumerate cameras and auto-select USB/external camera if no preference set.
 * Populates any camera-select dropdowns on the page.
 */
function detectAndSelectCamera(callback) {
  ...
  navigator.mediaDevices.enumerateDevices().then(function(devices) {
    ...
    // Populate all camera select dropdowns on the page
    var selects = document.querySelectorAll('.camera-select');
    ...
    // Auto-select external/USB camera if no preference saved
    if (!preferredCameraId && cameras.length > 1) {
      // Pick the camera that's NOT the built-in FaceTime/iSight camera
      var external = cameras.find(function(c) {
        var label = (c.label || '').toLowerCase();
        return !label.includes('facetime') && !label.includes('isight') && !label.includes('built-in') && !label.includes('integrated');
      });
      if (external) {
        preferredCameraId = external.deviceId;
        localStorage.setItem('preferredCameraId', preferredCameraId);
        // Update dropdowns
        selects.forEach(function(sel) { sel.value = preferredCameraId; });
        console.log('Auto-selected external camera:', external.label);
      }
    }
...
function onCameraSelectChange(sel) {
  preferredCameraId = sel.value;
  localStorage.setItem('preferredCameraId', preferredCameraId);
...
var crTesseractWorker = null;
var crTesseractWorkerLoading = false;
var crTesseractWorkerCallbacks = [];
var crSnLookupEmpId = '';
...
function initComputerRepair() {
  if (!crInitialized) {
    ...
    loadCrSettings();
    // Pre-populate camera dropdowns
    detectAndSelectCamera();
  }
  loadComputerRepairs();
}
...
// In Electron, CDN fetches hang. We load the main script from the local Express
// server AND configure createWorker with local workerPath, corePath, and langPath
// so that the Web Worker, WASM core, and trained data all come from localhost.

function loadTesseractJs(callback) {
  // If worker already created and ready, just callback
  if (crTesseractReady && crTesseractWorker) {
    if (callback) callback();
    return;
  }
  // If already loading, queue the callback
  if (crTesseractWorkerLoading) {
    if (callback) crTesseractWorkerCallbacks.push(callback);
    return;
  }

  function initWorker() {
    crTesseractWorkerLoading = true;
    console.log('Creating Tesseract worker with local paths...');

    // Create worker with all paths pointing to local Express server (relative URLs
    // work because the app now loads from http://localhost via Express)
    Tesseract.createWorker('eng', 1, {
      ...
      crTesseractWorker = worker;
      crTesseractReady = true;
      crTesseractWorkerLoading = false;
      console.log('Tesseract worker ready (local)');
      if (callback) callback();
      // Flush any queued callbacks
      var cbs = crTesseractWorkerCallbacks.slice();
      crTesseractWorkerCallbacks = [];
      cbs.forEach(function(cb) { cb(); });
    }).catch(function(err) {
      console.error('Tesseract local worker failed:', err);
      crTesseractWorkerLoading = false;
      showToast('OCR engine failed to load: ' + (err.message || err), 'error');
    });
  }

  // First, load the main Tesseract.js script if not already loaded
  if (typeof Tesseract !== 'undefined') {
    initWorker();
    return;
  }
  if (document.getElementById('tesseract-script')) {
    // Script tag exists but Tesseract not yet defined — wait
    ...
    setTimeout(function() {
      ...
      if (typeof Tesseract === 'undefined') {
        crTesseractWorkerLoading = false;
        showToast('OCR library failed to load (timeout).', 'error');
      }
    }, 15000);
    return;
  }
  ...
  script.onload = function() {
    console.log('Tesseract.js script loaded from local server');
    initWorker();
  };
  script.onerror = function() {
    console.log('Local tesseract script failed, trying CDN...');
    var cdnScript = document.createElement('script');
    cdnScript.src = 'https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js';
    cdnScript.onload = function() {
      console.log('Tesseract.js script loaded from CDN');
      initWorker();
    };
    cdnScript.onerror = function() {
      crTesseractWorkerLoading = false;
      showToast('Failed to load OCR library.', 'error');
    };
...
function computerRepairStartWebcam() {
  ...
  detectAndSelectCamera(function() {
    navigator.mediaDevices.getUserMedia({ video: getPreferredVideoConstraints() })
      ...
      .catch(function(err) {
        ...
        if (preferredCameraId) {
          console.log('Preferred camera failed, trying any camera:', err.message);
          navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 1280 }, height: { ideal: 720 } } })
            .then(function(stream) {
              ...
              video.srcObject = stream;
              video.play();
              showToast('Camera started (using default)', 'success');
            })
            .catch(function(err2) {
              showToast('Camera access denied: ' + err2.message, 'error');
            });
        } else {
...
function computerRepairRunOcr(dataUrl) {
  ...
  loadTesseractJs(function() {
    ...
    // Use the pre-created worker (all paths already local)
    crTesseractWorker.recognize(dataUrl).then(function(result) {
...
function computerRepairOpenServiceNow() {
  ...
  if (manufacturerModel) {
    // The actual ServiceNow field name has a typo: u_manufacturerer_model (3 e's)
    qParts.push('u_manufacturerer_model=' + manufacturerModel);
    qParts.push('u_manufacturer_model=' + manufacturerModel);
    qParts.push('u_make_model=' + manufacturerModel);
  ...
  var query = qParts.join('^');
  var url = 'https://pbcsd.service-now.com/incident.do?sys_id=-1&sysparm_query=' + encodeURIComponent(query);
  // Also pass manufacturer/model as direct URL params
  if (manufacturerModel) {
    url += '&u_manufacturerer_model=' + encodeURIComponent(manufacturerModel);
  }
...
function computerRepairPollForInc() {
  ...
  window.api.invoke('getLatestSnIncident', crSnLookupEmpId, crSnLookupShortDesc).then(function(result) {
      if (result && result.success && result.incidentNumber) {
        var incInput = document.getElementById('cr-inc-number');
        if (incInput) incInput.value = result.incidentNumber;
        if (incStatus) {
...
function computerRepairSave(sendNow) {
  ...
  window.api.invoke('createComputerRepair', data).then(function(result) {
      if (result && result.success) {
        ...
        if (sendNow) {
          ...
          window.api.invoke('sendComputerRepairEmail', repairId).then(function(emailResult) {
              if (emailResult && emailResult.success) {
          ...
          window.api.invoke('queueComputerRepairEmail', repairId).then(function(queueResult) {
              if (queueResult && queueResult.success) {
...
function loadComputerRepairs() {
  window.api.invoke('getComputerRepairs', 50).then(function(result) {
...
function runCrAiClassification(text) {
  ...
  window.api.invoke('classifyComputerRepair', text).then(function(result) {
      spinner.style.display = 'none';
      if (result && result.category) {
...
function loadCrSimilarIssues(text) {
  window.api.invoke('findSimilarComputerRepairs', text).then(function(result) {
...
function crUseSimilarIssue(issueId) {
  window.api.invoke('getComputerRepairById', issueId).then(function(result) {
...
function loadRepairTemplates() {
  window.api.invoke('getRepairTemplates').then(function(data) {
...
function selectRepairTemplate(templateId) {
  ...
  selectedRepairTemplate = {
    ...
    requiresSerial: !!template.requiresSerial && template.requiresSerial !== 'FALSE',
    requiresPhoto: !!template.requiresPhoto && template.requiresPhoto !== 'FALSE'
  };
  ...
  // Pre-fill the manufacturer/model field from template
  var deviceModelField = document.getElementById('mc-device-model');
  if (deviceModelField) {
    deviceModelField.value = selectedRepairTemplate.manufacturerModel || '';
  }
...
function templateOpenServiceNow() {
  ...
  // Read manufacturer/model from the form field (user may have edited it)
  var deviceModelField = document.getElementById('mc-device-model');
  var manufacturerModel = deviceModelField ? deviceModelField.value.trim() : (d.manufacturerModel || '');

  // Build description with device info
  var enrichedDesc = '';
  if (serialNumber) {
    enrichedDesc = '[S/N: ' + serialNumber + ' | Model: ' + manufacturerModel + (assetTag ? ' | Asset: ' + assetTag : '') + ']\n';
  }
...
function templateSave(sendNow) {
  ...
  var repairData = {
    ...
    serialNumber: serialNumber,
    computerModel: manufacturerModel,
  ...
  window.api.invoke('createComputerRepair', repairData).then(function(result) {
      if (result && result.success) {
...
/**
 * Start a new request — clears fields but keeps the same template selected.
 */
function templateNewRequest() {
  // Remember which template was selected
  var currentTemplate = selectedRepairTemplate;

  // Clear employee and all input fields
  mcEmployee = null;
  ...
  if (typeof maxCaseStopPhotoCamera === 'function') maxCaseStopPhotoCamera();
  if (mcWebcamStream) {
    mcWebcamStream.getTracks().forEach(function(t) { t.stop(); });
    mcWebcamStream = null;
  }
  var webcamVideo = document.getElementById('mc-webcam-video');
  ...
  var ocrStatus = document.getElementById('mc-ocr-status');
  if (ocrStatus) ocrStatus.style.display = 'none';

  // Switch serial entry back to manual mode
  if (typeof maxCaseSetSerialMode === 'function') maxCaseSetSerialMode('manual');

  // Keep the template selected — restore it
  selectedRepairTemplate = currentTemplate;

  // Re-populate manufacturer/model from template
  var deviceModelField = document.getElementById('mc-device-model');
  if (deviceModelField && currentTemplate) {
    deviceModelField.value = currentTemplate.manufacturerModel || '';
  }

  // Focus the employee ID field for the next entry
  var empInput = document.getElementById('mc-emp-id');
  if (empInput) {
    empInput.focus();
  }

  showToast('Form cleared — ready for new request', 'success');
}
...
function templateResetForm() {
  ...
  var fields = ['mc-emp-id', 'mc-serial-number', 'mc-warranty-date', 'mc-asset-tag', 'mc-inc-number', 'mc-device-model'];
  fields.forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.value = '';
  });
...
function renderTemplateSettingsList() {
  ...
  window.api.invoke('getRepairTemplates').then(function(allTemplates) {
...
function saveRepairTemplateFromEditor() {
  ...
  showToast('Saving template...', 'info');
  window.api.invoke('saveRepairTemplate', data).then(function(result) {
      if (result && result.success) {
...
function confirmDeleteRepairTemplate(templateId) {
  showConfirmCard({
    ...
    confirmText: 'Delete',
    onConfirm: function() {
      window.api.invoke('deleteRepairTemplate', templateId).then(function(result) {
          if (result && result.success) {
            showToast('Template deleted', 'success');
            loadRepairTemplates();
          } else {
      ...
      .catch(function(err) {
        ...
        navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 1280 }, height: { ideal: 720 } } })
          .then(function(stream) {
            ...
            video.srcObject = stream;
            video.play();
            showToast('Camera started (default).', 'info');
          })
          .catch(function(err2) {
            showToast('Camera error: ' + err2.message, 'error');
          });
      });
  });
}
...
function maxCaseRunOcr(dataUrl) {
  ...
  // Load Tesseract.js first, then run OCR using the pre-created worker
  loadTesseractJs(function() {
      ...
      .catch(function(err) {
        ...
        navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 1280 }, height: { ideal: 720 } } })
          .then(function(stream) {
            ...
            video.srcObject = stream;
            video.play();
            showToast('Camera ready (default).', 'info');
          })
          .catch(function(err2) {
            showToast('Camera error: ' + err2.message, 'error');
          });
...
function maxCaseOpenServiceNow() {
  ...
  qParts.push('serial_number=' + serialNumber);
  // The actual ServiceNow field name has a typo: u_manufacturerer_model (3 e's)
  qParts.push('u_manufacturerer_model=' + d.manufacturerModel);
  qParts.push('u_manufacturer_model=' + d.manufacturerModel);
  qParts.push('u_make_model=' + d.manufacturerModel);
  ...
  if (d.manufacturerModel) {
    url += '&u_manufacturerer_model=' + encodeURIComponent(d.manufacturerModel);
  }
...
function saveCrAiTrainingData(repairId, data) {
  ...
  window.api.invoke('saveCrTrainingEntry', trainingData).then(function() {
    ...
    },
    {
      ...
      tests: [
        ...
        { name: 'getDeviceTypeById()', desc: 'Fetch single device type', fn: function() {
          return new Promise(function(resolve) {
            window.api.invoke('getDeviceTypes').then(function(types) {
              if (!types || !types.length) return resolve({ pass: true, msg: 'No device types to test (skipped)' });
              window.api.invoke('getDeviceTypeById', types[0].id).then(function(dt) {
                resolve({ pass: !!dt, msg: dt ? 'Got: ' + (dt.name || dt.id) : 'Not found' });
              }).catch(function(e) { resolve({ pass: false, msg: e.message }); });
            }).catch(function(e) { resolve({ pass: false, msg: e.message }); });
          });
        }},
...
/**
 * Helper: wrap a window.api.invoke() call as a promise
 */
function serverCall(fnName, args, formatter) {
  return new Promise(function(resolve) {
    var callArgs = args && args.length ? [fnName].concat(args) : [fnName];
    window.api.invoke.apply(null, callArgs)
      .then(function(result) {
...
function fnExists(name) {
  return new Promise(function(resolve) {
    // In Electron, all IPC functions are available via window.api.invoke
    var exists = typeof window.api.invoke === 'function';
    resolve({ pass: exists, msg: exists ? 'IPC bridge available' : 'IPC bridge not found' });
  });
}
